 ...
 */
public abstract class FBClassifier<T, K> implements IFeatureProbability<T, K> {
     ...
     * they become too old.
     */
    private Queue<FBClassification<T, K>> memoryQueue;
    ...
    public void reset() {
        this.featureCountPerCategory =
                new Hashtable<K, Dictionary<T,Integer>>(
                        FBClassifier.INITIAL_CATEGORY_DICTIONARY_CAPACITY);
        this.totalFeatureCount =
                new Hashtable<T, Integer>(
                        FBClassifier.INITIAL_FEATURE_DICTIONARY_CAPACITY);
        this.totalCategoryCount =
                new Hashtable<K, Integer>(
                        FBClassifier.INITIAL_CATEGORY_DICTIONARY_CAPACITY);
        this.memoryQueue = new LinkedList<FBClassification<T, K>>();
    }
    ...
    public void learn(K category, Collection<T> features) {
        this.learn(new FBClassification<T, K>(features, category));
    }
     ...
     */
    public void learn(FBClassification<T, K> classification) {
        ...
        if (this.memoryQueue.size() > this.memoryCapacity) {
        	FBClassification<T, K> toForget = this.memoryQueue.remove();
     ...
     */
    public abstract FBClassification<T, K> classify(Collection<T> features);
